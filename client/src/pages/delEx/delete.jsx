            ...
            <div className="bg-white pt-32">
                <div className="flex flex-row w-screen justify-around">
                    <div className="flex flex-col border-1 border-black rounded-md bg-white bg-opacity-30 shadow-md shadow-gray-700 w-[800px] ">
                        ...
                        <input type="text" className='p-4 text-xl outline-none border-b' placeholder="Search exercises by name" value={exerciseSearch} onChange={(e) => setExerciseSearch(e.target.value)} />
                    ...
                    <div className="flex flex-col border-1 border-black rounded-md bg-white bg-opacity-30 shadow-md shadow-gray-700 w-[800px]">
                        ...
                        <input type="text" className='p-4 text-xl outline-none border-b' placeholder="Search users by name, email or id" value={userSearch} onChange={(e) => setUserSearch(e.target.value)} />
                        {users ? <UserList users={users.filter((user) =>
                            user.email.includes(userSearch) || user.name.includes(userSearch) || user._id.includes(userSearch)
                        )} /> : null}
                    </div>
                </div>
