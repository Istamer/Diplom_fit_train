...
function Calendar() {
    ...
    useEffect(() => {
        if (user) {
            console.log(user);
            const marked = user.workautDays.map(date => new Date(date).toLocaleDateString());
    ...
    for (let i = 1; i <= daysInMonth; i++) {
        const isMarked = markedDays.includes(
            new Date(date.getFullYear(), date.getMonth(), i).toLocaleDateString()
        );
        //console.log(isMarked)
        days.push(
            <div
                key={`current-${i}`}
                className={`${i === new Date().getDate()  && date.getMonth() === new Date().getMonth() ? "bg-blue-500 text-white" : ""
