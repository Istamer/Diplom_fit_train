...
const Bmicalc = () => {
    ...
    const addToBmiArray = async (bmiValue) => {
        const token = localStorage.getItem("TOKEN");
        console.log(bmiValue)
        await client.post(`/auth/users/bmi`, { bmi: bmiValue }, {
    ...
    const getBmiStat = async () => {
        try {
            const token = localStorage.getItem("TOKEN");
            const response = await client.get(`/auth/users/bmi`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    function calculateBMI(e) {
        ...
        if(!weight||!height||isNaN(Number(weight))||isNaN(Number(height)) || weight<=0 || height <=0){
            return alert("Перевірте коректність даних");
        }
    ...
    return (
        <div>
            <div className="flex justify-around w-full m-10 pb-20">
                <form className="bg-white shadow-lg rounded px-8 pt-6 pb-8 mb-4 w-[300px] h-[370px]" onSubmit={calculateBMI}>
